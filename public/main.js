...
function getZestimate() {
  ...
  fetch(`https://cors-anywhere.herokuapp.com/http://www.zillow.com/webservice/GetDeepSearchResults.htm?zws-id=X1-ZWz17qrfs9kp3f_17jma&address=${streetAddress}&citystatezip=${city}%2C+${state}`)
    ...
    .then(response => {
      ...
      let ZestimateAmt = xml.querySelector('amount').innerHTML
      // Convert to Number
    ...
    .catch(error => {
      ...
      document.querySelector("#zestimate").innerText = 'Sorry, Unable To Retrieve Zestimate From Zillow'
    })
}
...
trash.forEach((element) => {
  element.addEventListener('click', function() {
    const rawAddress = this.parentNode.parentNode.childNodes[9].innerText
